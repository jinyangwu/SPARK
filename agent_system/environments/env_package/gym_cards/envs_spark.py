...
class GymCardsWorker:
    ...
    def restore_to_state(self, replay_actions):
        # 逻辑：先重置，再重放动作历史
        self.action_history = []
...
class GymMultiProcessEnv(gym.Env):
    ...
    def step_with_mask(self, actions, active_mask):
        """
        Perform step in parallel.
        :param actions: list or numpy array, length must equal self.num_processes.
        :return: obs_list, reward_list, done_list, info_list
        ...
        for env_idx, action in zip(active_mask, actions):
            future = self.workers[env_idx].step.remote(action)
            futures.append(future)
    ...
    def restore(self, all_branches: Dict[int, List[int]]):
        # get parent action histories
        parents_idx = list(all_branches.keys())
        parents_action_history = self.get_action_history(parents_idx)

        futures = []
        for parent_idx in all_branches.keys():
            # restore child environments
            for child_idx in all_branches[parent_idx]:
                worker = self.workers[child_idx]
                futures.append(worker.restore_to_state.remote(parents_action_history[parent_idx]))
        ray.get(futures)

    def get_action_history(self, idxes: list[int]):
        """
        Get action history for environment at index idx.
        :param idx: Index of the environment (0 <= idx < self.num_processes)
        :return: action_history list
        """
        action_histories = {}
        futures = []
        for idx in idxes:
            worker = self.workers[idx]
            futures.append(worker.get_action_history.remote())

        results = ray.get(futures)
        for idx, result in zip(idxes, results):
            action_histories[idx] = result
