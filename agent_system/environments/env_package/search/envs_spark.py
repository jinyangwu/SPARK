...
class SearchMultiProcessEnv(gym.Env):
    ...
    def step_with_mask(self, actions: List[str], activte_indices: List[int]):
        # if len(actions) > self.batch_size:
        #     raise ValueError(f"Got {len(actions)} actions, but the env was initialized with total_envs={self.batch_size}")
        ...
        # padded_actions = list(actions) + [""] * pad_n
        # valid_mask = [True] * len(actions) + [False] * pad_n
        assert len(actions) == len(activte_indices), f"Expected {len(activte_indices)} actions for the active mask, got {len(actions)}"
        tasks = [
            self._loop.run_in_executor(self._executor, self._sync_step, self.envs[idx], act)
            for idx, act in zip(activte_indices, actions)
        ]
        results = self._loop.run_until_complete(asyncio.gather(*tasks))
    ...
    def sync_branch_states(self, all_branches: Dict[int, List[int]]):
        for parent_idx, childs_idx in all_branches.items():
            for child_idx in childs_idx:
                self.envs[child_idx] = deepcopy(self.envs[parent_idx])
