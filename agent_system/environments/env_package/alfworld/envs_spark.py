...
def worker_func(remote, config, seed, base_env):
    ...
    # Store action history for restore functionality
    action_history = []
    ...
    while True:
        ...
        ### core api for restoring to a certain state ###
...
class AlfworldEnvs(gym.Env):
    ...
    def step_with_mask(self, actions, active_mask):
        assert len(actions) == len(active_mask), \
            "The num of actions must be equal to the num of processes"

        # Send step commands to active workers only
        i2idx = {}
        for i, idx in enumerate(active_mask):
            remote = self.parent_remotes[idx]
            remote.send(('step', actions[i]))
            i2idx[i] = idx
        ...
        if self.multi_modal:
            # Note: This might need adjustment for partial environment updates
            image_obs_list = self.getobs()
        else:
            image_obs_list = None
    ...
    def restore(self, maps: list):
        """
        Restore environments to specific states based on action history.
        maps: [{"parent_idx": 0, "child_idx":0, "action_history": ['xx','xx']}]
        """
        for map_info in maps:
            ...
            self.prev_admissible_commands[child_idx] = self.prev_admissible_commands[parent_idx]
            
            # Send restore command to specific worker
        ...
        for map_info in maps:
            ...
            remote = self.parent_remotes[child_idx]
            obs, info = remote.recv()
            # Optionally process the restored observation if needed
    ...
    def close(self):
        ...
        for remote in self.parent_remotes:
            try:
                ...
                # Handle case where pipe is already closed
                pass
                
        for worker in self.workers:
            worker.join(timeout=5.0)  # Add timeout to avoid hanging
            if worker.is_alive():
                worker.terminate()
