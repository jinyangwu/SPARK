...
@ray.remote(num_cpus=0.2, memory=256*1024*1024)
class AlfworldWorker:
    ...
    ## add by dynamic
    def restore_to_state(self, action_history):
        """恢复到指定状态"""
        # 重放动作历史
...
class AlfworldEnvs(gym.Env):
    ...
    def step_with_mask(self, actions, active_mask):
        ...
        for i, (obs, scores, dones, info) in enumerate(results):
            ...
            self.prev_admissible_commands[i2idx[i]] = info['admissible_commands']
            rewards_list.append(compute_reward(info, self.multi_modal))
    ...
    def restore(self, maps: list):
        ...
        for map in maps:
            ...
            action_history = map['action_history']
            # envs只需要更新self.prev_admissible_commands
            self.prev_admissible_commands[child_idx] = self.prev_admissible_commands[parent_idx]
            future = self.workers[child_idx].restore_to_state.remote(action_history)
            futures.append(future)
        results = ray.get(futures)
        # for result in results:
        #     print('restored obs', result[0])
