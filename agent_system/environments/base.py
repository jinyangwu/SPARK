...
class EnvironmentManagerBase:
    ...
    def step(self, text_actions: List[str], activate_indices: List[int] = None) -> Tuple[Dict[str, Any], np.ndarray, np.ndarray, List[Dict]]:
        """
        ...
        if activate_indices is not None:
            next_obs, rewards, dones, infos = self.envs.step_with_mask(actions, activate_indices)
        else:
