    ...
    def fetch_rlvmr(
        ...
        for env_idx in range(self.batch_size):
            ...
            for j, rec in enumerate(recent):
                ...
                obs = rec[obs_key]
                lines.append(
                    f"[Step {step_num}, Observation {step_num}: '{obs}', Action {step_num}: '{act}']"
                )
            action_history = "\n".join(lines)

            # for thinking history
            history_think_length = min(max_reasoning_steps, len(self._data[env_idx]))
            recent_think = self._data[env_idx][-history_think_length:]
            start_idx_think = len(self._data[env_idx]) - history_think_length
            action_history += "\n- recent reasoning process: \n"
            for j, record in enumerate(recent_think):
                step_num = start_idx_think + j + 1
                obs = record[obs_key]
                reasoning = record[reasoning_key]
                action_history += f"[Step {step_num}, Output: '{reasoning}']\n"

            memory_contexts.append(action_history)
            valid_lengths.append(valid_len)
