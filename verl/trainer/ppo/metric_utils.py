...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    metrics = {
        ...
        **({f"episode/{k}": v[0].item() for k, v in batch.non_tensor_batch.items() if ("success_rate" in k or "custom" in k)}),
    }
    return metrics
...
def compute_throughout_metrics(batch: DataProto, timing_raw: Dict[str, float], n_gpus: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "perf/total_num_res_tokens": sum(batch.meta_info["global_res_token_num"]),
